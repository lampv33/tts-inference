...
def build_model(args, n_symbols, bert):
    ...
    text_encoder = TextEncoder(channels=args.hidden_dim, kernel_size=5, depth=args.n_layer, n_symbols=n_symbols)
    ...
    bert_encoder = BertEncoder(bert.config.hidden_size, args.hidden_dim)
...
def build_voice_encoder(args):
    style_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)  # acoustic style encoder
    predictor_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)  # prosodic style encoder
    nets = Munch(style_encoder=style_encoder, predictor_encoder=predictor_encoder)
    return nets
