...
ref_huonglinh_thoisu = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/huonglinh-thoisu/xRHvzHGScB4_0.wav'
...
ref_web5n = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/web5n/1 - Thiếu Tự Tin？ Xài Ngay Chiêu Này! (Rất Dễ)_1.wav'
ref_trongkhuong_nguoithanhcong = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/trongkhuong-nguoithanhcong/0hjffV6j5BE_0.wav'

ref_azaudiobook = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/azaudiobook/DqRYA2mGCW0_0.0_60.wav'
ref_beyondlimit_ngalevi = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/beyondlimit-ngalevi/3wVLylGgVag_0.0_0.wav'
#ref_beyondlimit_ngalevi = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/beyondlimit-ngalevi/7faZb5Y0Q_Y_0.0_0.wav'
ref_phephim = '/mnt/nfs-shared/kilm/users/lampv/prepare_data_tts/segments/phephim/_SHH85xzucM_0.wav'
...
def prepare_phuonglinh(style, model):
    ...
    if style == 'news':
        ref_embedding = model.compute_ref_emb_mix(ref_huonglinh_thoisu, ref_huonglinh_thoisu, ref_hieubiethon, ref_hieubiethon)
        ...
        ref_embedding = model.compute_ref_emb_mix(ref_huonglinh_betterversion, ref_phephim, ref_prosody_audio_path_2=ref_huonglinh_phephim, ratio=3)
        text_demo = 'trong tư_duy , những công_nhân nam_giới dù trước đó ủng_hộ nhưng ngay khi lợi_ích của mình bị tổn_hại , họ lại đổ lỗi lên cuộc biểu_tình của những cô_gái với lời_lẽ cay_nghiệt . dẫu vậy , chẳng có gì có thể cản được tiếng_nói của những người phụ_nữ mạnh_mẽ này vang xa khắp thế_giới'
    elif style == 'podcast_energetic':
        ref_embedding_1 = model.compute_ref_emb_mix(ref_huonglinh_phephim, ref_huonglinh_phephim, ref_hieubiethon, ref_hieubiethon, ratio=3)
        ref_embedding_2 = model.compute_ref_emb_mix(ref_huonglinh_spiderum, ref_huonglinh_spiderum, ref_hieubiethon, ref_hieubiethon, ratio=3)
...
def prepare_thuydung(style, model):
    if style == 'news':
        ...
        ref_embedding = model.compute_ref_emb_mix(ref_numienbac, ref_hieubiethon, ref_prosody_audio_path_2=ref_hoangthuy_thoisu, ratio=3)
        text_demo = 'trước khi trở_thành biểu_tượng của một gia_tộc danh_giá , họ bắt_đầu từ đâu . làm thế nào một cậu bé lớn lên trong cảnh nghèo_khó lại có thể xây_dựng nền đế_chế tài_chính kinh_doanh khổng_lồ có phần định_hình nên lịch_sử hoa_kỳ'
...
def prepare_minhtu(style, model):
    if style == 'news':
        ...
        ref_embedding = (ref_embedding_1 + ref_embedding_2) / 2
        text_demo = 'Đôi khi việc đối_diện sự_thật về cảm_xúc của mình có thể khiến bạn lo_lắng . Bạn cần lòng can_đảm và trung_thực để có thể liên_lạc với các cảm_xúc và khuynh_hướng của mình . Hãy kiên_nhẫn và tin_tưởng bản_thân ngay cả khi bạn chỉ mới đạt những bước_tiến nhỏ nhất .'
